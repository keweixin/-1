<template>
  <div class="space-y-8">
    <div>
      ...
      <p class="mt-2 text-sm text-gray-500">管理您的商户信息与安全设置。</p>
    </div>
    ...
    <div class="bg-white rounded-3xl border border-gray-100 shadow-sm p-8">
      ...
      <div class="flex items-center gap-6 mb-6">
        <div class="h-20 w-20 overflow-hidden rounded-2xl border-2 border-gray-200 bg-gray-100 flex items-center justify-center text-2xl font-black text-blue-600">
          {{ profileInitial }}
        </div>
        <div>
          ...
          <p class="text-xs text-blue-600 font-medium mt-1">商户</p>
        </div>
      </div>
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label class="space-y-2 block">
          ...
          <input v-model="profileForm.nickname" class="w-full rounded-2xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-blue-500" />
        </label>
        <label class="space-y-2 block">
          <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">手机号</span>
          <input v-model="profileForm.phone" class="w-full rounded-2xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-blue-500" />
        </label>
        <label class="space-y-2 block">
          <span class="text-xs font-bold text-gray-400 uppercase tracking-wider">邮箱</span>
          <input v-model="profileForm.email" class="w-full rounded-2xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-blue-500" />
        </label>
      </div>
      <div class="flex justify-end mt-6">
        <button class="px-6 py-3 rounded-2xl bg-blue-600 text-white font-bold hover:bg-blue-700 transition-all disabled:bg-blue-300"
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
        <label class="space-y-2 block">
          ...
          <input v-model="pwdForm.oldPassword" type="password" placeholder="请输入原密码"
            class="w-full rounded-2xl border border-gray-200 px-4 py-3 text-sm outline-none focus:border-blue-500" />
        </label>
        <label class="space-y-2 block">
...
const profileInitial = computed(() => {
  const name = userInfo.value.nickname || userInfo.value.username || '商'
  return name[0] || '商'
})
